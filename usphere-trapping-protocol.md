...
**Hardware:** Shaker piezo (PA4CEW, attached to glass slide), Keysight 33511B AWG, external voltage-controlled amplifier
**Automation:** `procedures/proc_shake_dropper.py`
**Module:** `modules/mod_keysight_awg.py`

### Shaking mechanism

A PA4CEW 2×2×2 mm piezo stack chip (Thorlabs, 150 V, same unit as used in the dropper construction) is epoxied to the glass slide.  When driven by the AWG through the amplifier, the piezo vibrates the glass strip mechanically, dislodging microspheres from the underside of the tip so that they fall through the aperture and into the trapping beam.

The shaker waveform is a **continuous frequency sweep**: the AWG outputs a sine wave that sweeps linearly from a start frequency to a stop frequency over a fixed sweep time, then immediately repeats.  Sweeping over a band of frequencies (rather than driving a single tone) excites a broader range of resonance modes in the slide-and-piezo assembly, giving more robust release across different sphere positions on the tip.

### AWG hardware

**Instrument:** Keysight 33511B (single-channel) AWG, LAN-connected.
**VISA address (lab unit):** `TCPIP0::K-33511B-01756::inst0::INSTR`
**Python driver:** [`resources/KEYSIGHT33500B/`](resources/KEYSIGHT33500B/) (git submodule, Moore-Lab/KEYSIGHT33500B).
**Module:** [`modules/mod_keysight_awg.py`](modules/mod_keysight_awg.py)

The module inserts the submodule directory into `sys.path` at load time so `KS33500BController` can be imported without a separate package install.

### External amplifier

The AWG signal is amplified before reaching the piezo.  The amplifier has a voltage-controlled gain interface over RS-232 (command format `VSET1: %.2f`) on COM5 of the host PC.  In the current software implementation, amplitude stepping is applied directly to the AWG output level — the amplifier gain is left at a fixed operating point.  If the fixed amplifier gain proves insufficient, a `mod_dropper_amplifier` module should be added to ramp the COM5 gain in sync with the AWG steps.

### Shaking parameters

The shaking sequence steps the AWG amplitude upward in equal increments, dwelling at each amplitude for a fixed time.  Default values (from the Stage 5 LabVIEW VI):

| Parameter | Default | Description |
|---|---|---|
| Start amplitude | 0.10 Vpp | Initial AWG output amplitude |
| Step size | 0.05 Vpp | Amplitude increment per step |
| Number of steps | 50 | Total steps before sequence ends |
| Dwell time | 5.0 s | Time held at each amplitude before stepping |
| Start frequency | 100 kHz | Lower bound of the frequency sweep |
| Stop frequency | 700 kHz | Upper bound of the frequency sweep |
| Sweep time | 0.1 s | Period of one frequency sweep cycle |

Maximum amplitude reached: 0.10 + 50 × 0.05 = **2.60 Vpp**.

The operator presses **Stop** as soon as a sphere is confirmed trapped (see Stage 6).  The sequence does not need to complete all steps; it is common for a sphere to be captured at a low amplitude step.

### Procedure workflow

1. Confirm the dropper is at the `dropping` preset (Stage 4 complete).
2. Open the **Shake Dropper** procedure tab and enter the VISA address.
3. Click **Connect** to verify the AWG is reachable.
4. Adjust sweep and amplitude-ramp parameters if needed.
5. Click **▶ Start Shaking**.  The procedure:
   - Configures the AWG sweep (setup_sweep) and enables output.
   - Steps the amplitude once per dwell interval, logging each step.
   - Monitors the BPD signal from the FPGA (displayed live in the procedure tab).
6. When a sphere is trapped (confirmed on BPD or camera), click **■ Stop**.
   - The AWG output is disabled immediately after the current dwell completes.
7. Proceed to Stage 6 for confirmation, then Stage 7 to retract the dropper.
